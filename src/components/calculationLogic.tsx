export const calculationLogic = (
  ...
  let amountToPay =
    (amount *
      (ratePercent * Math.pow(1 + ratePercent, Math.round(numberOfPayments)))) /
    (Math.pow(1 + ratePercent, Math.round(numberOfPayments)) - 1);
  let totalToPay =
    Math.round(amountToPay * Math.round(numberOfPayments) * 100) / 100;
